...
cd /d ../..
set basePath=%cd%
set publishPath=%basePath%/Publish/release/Sers/CL压测
...
for %%i in (netcoreapp2.1,net6.0) do (  
	...
	::单体压测
	set targetPath=%basePath%/Publish/release/Sers/Sers压测/sers压测-单体压测!netVersion!
	...
	::分布式压测
	set targetPath=%basePath%/Publish/release/Sers/Sers压测/sers压测-分布式压测!netVersion!

...
namespace Sers.Core.Util.ConfigurationManager
{
    public class JsonFile
    {
        ...
        {
            root = LoadJsonFile(configPath) ?? new JObject();
        }
        ...
        /// <returns></returns>
        private static JToken LoadJsonFile(string FilePath)
        {
            try
            {
                return JsonConvert.DeserializeObject(File.ReadAllText(FilePath)) as JToken;

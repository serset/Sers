...
private:
	...
	static void LogTxt(const  string & level,const  string &  finalMsg){
		...
		string foldPath=CommonHelp::GetAbsPathByRealativePath("Logs")+"/" + CommonHelp::TimeToString("%Y-%m");
		string logFilePath = foldPath+"/["+CommonHelp::TimeToString("%Y-%m-%d")+"]"+level+".log";

		//(x.2) make sure the fold  exits
		if(access(foldPath.data(), F_OK) ==-1)
		{
			mkdir(foldPath.data(),0755);
		}

...
namespace Sers.Gover.Core
{
    ...
    public class ApiStationMng
    {
        ...
        static bool Config_PrintWhenRegistApi = (true == ConfigurationManager.Instance.GetByPath<bool?>("Sers.ServiceCenter.PrintWhenRegistApi"));

        void ApiNode_Add(string route, ApiNode apiNode)
        {
            try
            {
                if (Config_PrintWhenRegistApi)
                {

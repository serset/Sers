namespace Sers.CL.Zmq.FullDuplex.Zmq
{
	...
	public static unsafe class zmq
	{
        ...
        //{
        //    zmq_msg_t* msgToSend = stackalloc zmq_msg_t[1];
        //    msg_init_size((IntPtr)msgToSend, byte_send.Length);
        //    var ptrToSend = msg_data((IntPtr)msgToSend);
        //    Marshal.Copy(byte_send, 0, (IntPtr)ptrToSend, byte_send.Length);
        //    int ret = msg_send((IntPtr)msgToSend, (IntPtr)socket, flags);
        //    int result = msg_close((IntPtr)msgToSend);
        ...
        public static void ReceiveMessage(IntPtr socket, out byte[] byte_received,int flags=0)
        {
            ...
            int rec_ionti = msg_init((IntPtr)msgReceived);
            int size;
            while (-1 == (size = zmq.msg_recv((IntPtr)msgReceived, socket, flags)))
            {
                int errno = zmq.errno();
            ...
            var ptr = msg_data((IntPtr)msgReceived);
            byte_received = new byte[size];

            Marshal.Copy((IntPtr)ptr, byte_received, 0, byte_received.Length);
            //string str = Marshal.PtrToStringAnsi((IntPtr)ptr);

            int result = msg_close((IntPtr)msgReceived);
        }

        [StructLayout(LayoutKind.Sequential)]
        unsafe struct zmq_msg_t
        {
        ...
        public static void SendMessage(IntPtr socket, byte[] buffer, int flags, int offset, int count)
        {
            ...
            int length;
            while (-1 == (length = zmq.send(socket, pinPtr, count, flags)))
            {
                int errno = zmq.errno();
                if (errno == (int)ZError.EINTR)
                {

...
namespace Sers.Mq.Socket.Iocp
{
    public class ClientMq: IClientMq
    {
        ...
        public ClientMq()
        {
            ...
            _mqConn.Conn_OnDisconnected = MqConn_OnDisconnected;
        ...
        public bool Connect()
        {
            try
            {
                Logger.Info("[ServerMq] Socket.Iocp,connecting... host:" + host + " port:" + port);
                ...
                {
                    Logger.Info("[ClientMq] Socket.Iocp,connected.");
                    return true;
                }
            }
            catch (Exception ex)
        ...
        private void ProcessReceive(SocketAsyncEventArgs e)
        {
            try
            {
                ...
                {
                    //读取数据                  
                    _mqConn.AppendData(new ArraySegment<byte>(e.Buffer, e.Offset, e.BytesTransferred));

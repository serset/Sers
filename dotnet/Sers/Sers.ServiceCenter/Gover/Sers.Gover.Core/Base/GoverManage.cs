...
using Sers.Core.Station.Module.SsApiDiscovery.ApiVerifyRequest;
...
namespace Sers.ServiceCenter.ApiCenter.Gover.Core
{
    ...
    public class GoverManage : IApiCenterManage
    {
        ...
        public void CallApiAsync(IRpcContextData rpcData, ApiMessage requestMessage, Object sender, Action<object, List<ArraySegment<byte>>> callback)
        {
            try
            {
                #region (x.1)route 判空               
                ...
                if (!ApiRpcVerify2.Verify(rpcData.oriJson, apiNode.apiDesc.rpcVerify2, out var verifyError))
                {
            ...
            void SendReply(SsError error)
            {
                ...
                callback(sender, new ApiMessage().InitByError(error).Package());
            }
        ...
        public void ServiceStation_Remove(IMqConn conn)
        {
        ...
        {
            var serviceStation = serviceStationMng.ServiceStation_Pause(connGuid);
            if (serviceStation != null)
            {
        ...
        {
            var serviceStation = serviceStationMng.ServiceStation_Start(connGuid);
            if (serviceStation != null)
            {
